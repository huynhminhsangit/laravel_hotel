...
class Feed
{
    public static function read($data = [], $page = 1)
    {
        ...
        foreach ($data as $item) {
            switch ($item['source']) {
                case 'vnexpress':
                    if (self::checkSourceLink($item['source'], $item['link'])) $items = array_merge_recursive($items, self::readVNExpress($item['link']));
                    break;
                case 'cafebiz':
                    if (self::checkSourceLink($item['source'], $item['link'])) $items = array_merge_recursive($items, self::readCafeBiz($item['link']));
                    break;
                case 'tuoitre':
                    if (self::checkSourceLink($item['source'], $item['link'])) $items = array_merge_recursive($items, self::readTuoiTre($item['link']));
                    break;
            }
        }

        $position = ($page - 1) * $limit;
        $result = array_slice($items, $position, $limit);
